...
namespace Pets.Domain.Validations
{
    ...
    {
        public ContractValidations<T> DocumentIsValid(Document document, string message, string porpertyName)
        {
            if (document.DocumentType == Enums.EDocumentType.CPF)
                if (!IsCpf(document.DocumentNumber))
                    AddNotification(new Notification(message, porpertyName + " CPF"));

            if (document.DocumentType == Enums.EDocumentType.CNPJ)
                if (!IsCnpj(document.DocumentNumber))
                    AddNotification(new Notification(message, porpertyName + " CNPJ"));
